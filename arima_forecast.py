import os
import numpy as np
import polars as pl
from datetime import datetime, timezone
from statsmodels.tsa.statespace.sarimax import SARIMAX

from binance import Binance
from telegram import TelegramNotifier

class ARIMAForecast:
    def __init__(self):
        self.interval = os.getenv("INTERVAL", "4h")
        self.limit = int(os.getenv("LIMIT", 200))
        self.steps = int(os.getenv("STEPS", 30))
        self.symbols = [s.strip().upper() for s in os.getenv("SYMBOLS", "DOTUSDT").split(",")]
        self.threshold = float(os.getenv("PRICE_CHANGE_THRESHOLD", 2))
        self.previous_results = {}

        if not self.symbols:
            print("[WARN] The symbols is missing in .env")

    # ---- Auto ARIMA grid search ----
    def _auto_arima_grid_search(self, y, p_range=(0, 3), d_range=(0, 2), q_range=(0, 3)):
        best_aic = np.inf
        best_order = None
        best_model = None

        for d in d_range:
            for p in range(p_range[0], p_range[1] + 1):
                for q in range(q_range[0], q_range[1] + 1):
                    try:
                        model = SARIMAX(
                            y, order=(p, d, q),
                            enforce_stationarity=False,
                            enforce_invertibility=False
                        )
                        res = model.fit(disp=False)
                        if res.aic < best_aic:
                            best_aic = res.aic
                            best_order = (p, d, q)
                            best_model = res
                    except Exception:
                        continue

        print(f"[INFO] Selected ARIMA order={best_order} with AIC={best_aic:.2f}")
        return best_model, best_order

    # ---- Run ARIMA forecast ----
    def _run_auto_arima_forecast(self, df: pl.DataFrame):
        # Ensure df sorted by date
        df = df.sort("open_time")

        y = np.log(df["close"].to_numpy())

        model, order = self._auto_arima_grid_search(y)
        if model is None:
            raise ValueError("[ERROR] No valid ARIMA model found during grid search.")

        forecast = model.get_forecast(steps=self.steps)
        pred_mean = np.exp(forecast.predicted_mean)
        ci = np.exp(forecast.conf_int())

        freq_guess = "1h"
        last_time = df["open_time"].to_pandas().iloc[-1]
        future_index = pl.Series("date", 
                                 pl.date_range(low=last_time, 
                                               high=last_time + pd.Timedelta(f"{self.steps}h"), 
                                               interval=freq_guess, 
                                               eager=True)[1:])

        fc = pl.DataFrame({
            "date": future_index,
            "predicted_price": pred_mean,
            "lower_ci": ci.iloc[:, 0].values,
            "upper_ci": ci.iloc[:, 1].values
        })

        fc = fc.with_columns(
            (pl.col("predicted_price").pct_change() * 100).alias("pct_change")
        )

        return fc

    # ---- Main run loop ----
    def run(self):
        now = datetime.now(timezone.utc)
        notifier = TelegramNotifier()
        binance = Binance()

        for symbol in self.symbols:
            print(f"\n[INFO] Starting forecast for {symbol} at {now}")

            df = binance.klines(symbol, self.interval, self.limit)
            df = pl.from_pandas(df)  # Convert to Polars DataFrame

            current_price = df["close"][-1]
            prev_price = self.previous_results.get(symbol)
            percent_change = None

            if prev_price is not None:
                percent_change = ((current_price - prev_price) / prev_price) * 100

            self.previous_results[symbol] = current_price

            print(f"📢 [INFO] Last actual price: {current_price:.4f}")

            forecast = self._run_auto_arima_forecast(df)
            forecast = forecast.filter(pl.col("date") > now)

            top5 = forecast.head(5)
            print("\n[INFO] Top 5 forecasted prices:")
            print(top5.select(["date", "predicted_price", "pct_change"]))

            message_lines = [f"📢 *ARIMA Forecast for {symbol}*"]
            if percent_change is not None:
                message_lines.append(f"Last actual price: {current_price:.4f} 🔥 Change: {percent_change:.2f}%")
            else:
                message_lines.append(f"Last actual price: {current_price:.4f}")

            for row in top5.iter_rows(named=True):
                pct = row["pct_change"]
                if pct is None:
                    label = "Hold 🚫"
                elif pct > self.threshold:
                    label = "Buy ✅"
                elif pct < -self.threshold:
                    label = "Sell 🎯"
                else:
                    label = "Hold 🚫"

                line = f"{row['date']:%d/%m/%y %H:%M}: {row['predicted_price']:.4f} ({pct:+.2f}%) - {label}"
                message_lines.append(line)

            message = "\n".join(message_lines)
            notifier.send(message)
